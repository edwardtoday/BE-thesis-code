...
void H264toMVCPacker::appendNAL(char *buf, int size)
{
	...
	case NAL_UNIT_CODED_SLICE_IDR_SCALABLE:
		...
		cout << endl;
		sp.timeId = sp.timeId * sps.viewCodingOrder.size() + sp.viewId;
		sp.viewId = 0;

		for (int i = 0; i < sp.list0.size(); ++i)
		{
			sp.list0[i] = makeRefFrameType(sp.list0[i].frameNum, sp.list0[i].newFrameNum, sp.list0[i].timeId * sps.viewCodingOrder.size()+sp.list0[i].viewId, 0);
		}

		for (int i = 0; i < sp.list1.size(); ++i)
		{
			sp.list1[i] = makeRefFrameType(sp.list1[i].frameNum, sp.list1[i].newFrameNum, sp.list1[i].timeId * sps.viewCodingOrder.size()+sp.list1[i].viewId, 0);
		}

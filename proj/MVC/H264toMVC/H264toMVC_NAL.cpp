...
void H264toMVCPacker::readSequenceParameterSet()
{
	...
	//ASSERT(ReadFlag() == 0);//constraint_set4_flag=0
	Comment("SPS: constrained_set5_flag");
	sps.constrained_set5_flag = ReadFlag()!=0;
	//ASSERT(ReadFlag() == 0);//constraint_set5_flag=0
	...
	sps.gaps_in_frame_num_value_allowed_flag = (ReadFlag()==1);//gaps_in_frame_num_value_allowed_flag=1
...
void H264toMVCPacker::readPictureParameterSet()
{
	...
	pps.seqParameterId = ReadUvlc();//seq_parameter_set_id
...
int H264toMVCPacker::readSliceHeader(SliceHeader &sp)
{
				...
				do {
					...
					else if (memory_management_control_operation==1)
					{
						...
						for (int x = 0; x < ref_frames[sp.viewId].size(); ++x)
							if (ref_frames[sp.viewId][x].second == fn)
							{
	...
	else if (sp.sliceType%5 == 1)
	{
		for (int i = 0; i < ref_frames[sp.viewId].size(); ++i)
			sp.list0.push_back(makePair(ref_frames[sp.viewId][i], sp.viewId));
		sp.list1 = sp.list0;
		for (int i = sp.list0.size() - 1; i >= 0; --i)
		{
			for (int j = 0; j < i; ++j)
				if (sp.list0[j].first.first>=sp.timeId && sp.list0[j+1].first.first<sp.timeId ||
					sp.list0[j+1].first.first<sp.timeId && sp.list0[j+1].first.first>sp.list0[j].first.first ||
					sp.list0[j+1].first.first>=sp.timeId && sp.list0[j+1].first.first<sp.list0[j].first.first)
				{
					Pair<Pair<int, int>, int> tmp = sp.list0[j]; sp.list0[j] = sp.list0[j+1]; sp.list0[j+1] = tmp;
				}
		}
		for (int i = sp.list1.size() - 1; i >= 0; --i)
		{
			for (int j = 0; j < i; ++j)
				if (sp.list1[j].first.first<=sp.timeId && sp.list1[j+1].first.first>sp.timeId ||
					sp.list1[j+1].first.first<=sp.timeId && sp.list1[j+1].first.first>sp.list1[j].first.first ||
					sp.list1[j+1].first.first>sp.timeId && sp.list1[j+1].first.first<sp.list1[j].first.first)
				{
					Pair<Pair<int, int>, int> tmp = sp.list1[j]; sp.list1[j] = sp.list1[j+1]; sp.list1[j+1] = tmp;
				}
		}
	}
	else if (sp.sliceType%5 == 0)
	{
		...
		for (int i = sp.list0.size() - 1; i >= 0; --i)
			for (int j = 0; j < i; ++j)
			{
				if (sp.list0[j].first.first <
					sp.list0[j+1].first.first)
				{
					Pair<Pair<int, int>, int> tmp = sp.list0[j]; sp.list0[j] = sp.list0[j+1]; sp.list0[j+1] = tmp;
				}
			}
	...
	if (sp.isAnchor)
	{
		for (int i = 0; i < sps.anchorRefsList0[sp.viewId].size(); ++i)
			sp.list0.push_back(makePair(sp.timeId, sps.anchorRefsList0[sp.viewId][i]));
		for (int i = 0; i < sps.anchorRefsList1[sp.viewId].size(); ++i)
			sp.list1.push_back(makePair(sp.timeId, sps.anchorRefsList1[sp.viewId][i]));
	}
	else
	{
		for (int i = 0; i < sps.nonAnchorRefsList0[sp.viewId].size(); ++i)
			sp.list0.push_back(makePair(sp.timeId, sps.nonAnchorRefsList0[sp.viewId][i]));
		for (int i = 0; i < sps.nonAnchorRefsList1[sp.viewId].size(); ++i)
			sp.list1.push_back(makePair(sp.timeId, sps.nonAnchorRefsList1[sp.viewId][i]));
	}
	*/
...
#define RPLR_VIEW_POS 5
	...
	if (ref_pic_list_reordering_flag0)
	{
		...
		{   
			...
			{   
				...
				//----- get frame -----   
				Pair<int, int> ft = makePair(-1, -1);
				for (int x = 0; x < ref_frames[sp.viewId].size(); ++x)
					if (ref_frames[sp.viewId][x].second == uiIdentifier)
					{
					...
					for (int x = uiIndex; x < sp.list0.size(); ++x)
						if (sp.list0[x].first.first == ft.first && sp.list0[x].second == sp.viewId)
						{
							sp.list0.erase(x);
							break;
						}
						sp.list0.insert(uiIndex, makePair(ft, sp.viewId));
						uiIndex++;   
			} // short-term   
			/*
			else // 4 or 5   
			{   
				...
				for (int x = 0; x < sp.list0.size(); ++x)
					if (sp.list0[x].first == sp.timeId && sp.list0[x].second == targetViewId)
					{   
						sp.list0.erase(x);
						break;
					}  
					sp.list0.insert(uiIndex, makePair(sp.timeId, targetViewId));
		...
		{   
			...
			{   
				...
				for (int x = 0; x < ref_frames[sp.viewId].size(); ++x)
					...
					int p = -1;
					for (int x = uiIndex; x < sp.list1.size(); ++x)
						if (sp.list1[x].first.first == ft.first && sp.list1[x].second == sp.viewId)
						{
							sp.list1.erase(x);
							break;
						}
						sp.list1.insert(uiIndex, makePair(ft, sp.viewId));
						uiIndex++;   
			} // short-term   
			/*
			else // 4 or 5   
			{   
				...
				for (int x = 0; x < sp.list1.size(); ++x)
					if (sp.list1[x].first == sp.timeId && sp.list1[x].second == targetViewId)
					{   
						sp.list1.erase(x);
						break;
					}   
					sp.list1.insert(uiIndex, makePair(sp.timeId, targetViewId));
	...
	if (nal_ref_idc)
	{
		if (sp.IDR)
		{
			...
			if (long_term_reference_flag)
			{
				...
				ref_frames[sp.viewId].push_back(makePair(sp.timeId, sp.frameNum));
		}
		else
		{
	...
	{
		printf(" REF (%d[%d],%d)\n", sp.list0[i].first.first, sp.list0[i].first.second, sp.list0[i].second);
	}
	printf("REF 1 Count = %d\n", sp.list1.size());
	for (int i = 0; i < sp.list1.size(); ++i)
	{
		printf(" REF (%d[%d],%d)\n", sp.list1[i].first.first, sp.list1[i].first.second, sp.list1[i].second);
	}
	/*
...
int32_t H264toMVCPacker::writeSubsetSequenceParameterSet()
{
	Comment("NALU HEADER: forbidden_zero_bit");
	WriteFlag(0);
	Comment("NALU HEADER: nal_ref_idc");
	...
	WriteUvlc(sps.viewCodingOrder.size() - 1);
...
void H264toMVCPacker::writeSlicePrefix()
{
	Comment("NALU HEADER: forbidden_zero_bit");
	WriteFlag(0);
	Comment("NALU HEADER: nal_ref_idc");
...
int H264toMVCPacker::writeSliceHeader(SliceHeader &sp)
{
	...
	if (nal_unit_type == NAL_UNIT_CODED_SLICE_SCALABLE)
	{
		sp.sliceType %= 5; // Not changed for scalable
		Comment("NALU HEADER: svc_mvc_flag");
		WriteFlag(0);
		Comment("NALU HEADER: non_idr_flag");
		WriteFlag(!sp.IDR);
	...
	if (sp.sliceType==0 || sp.sliceType==1 || sp.sliceType==5 || sp.sliceType==6 || sp.sliceType==3 || sp.sliceType==8 )
	{
...
#define RPLR_VIEW_POS 5
		...
		for (int i = 0; i < sp.list0.size(); ++i)
		{
			if (sp.list0[i].first.first==sp.timeId)
			{
				int vId = 0;
				if (sp.isAnchor)
				{
					for (int j = 0; j < sps.anchorRefsList0[sp.viewId].size(); ++j)
						if (sps.anchorRefsList0[sp.viewId][j] == sp.list0[i].second)
						{
							vId = j;
							break;
						}
				...
				else
				{
					for (int j = 0; j < sps.nonAnchorRefsList0[sp.viewId].size(); ++j)
						if (sps.nonAnchorRefsList0[sp.viewId][j] == sp.list0[i].second)
						{
							vId = j;
							break;
						}
			...
			{
				if (sp.list0[i].first.second>picNum)
				{
					WriteUvlc(RPLR_POS);
					WriteUvlc(sp.list0[i].first.second - picNum - 1);
				}
				else
				{
					WriteUvlc(RPLR_NEG);
					WriteUvlc(picNum - sp.list0[i].first.second - 1);
				}
				picNum = sp.list0[i].first.second;
			}
		}
		WriteUvlc(RPLR_END);
		if (sp.sliceType==1 || sp.sliceType==6) // B, EB
		{
			...
			for (int i = 0; i < sp.list1.size(); ++i)
			{
				if (sp.list1[i].first.first==sp.timeId)
				{
					int vId = 0;
					if (sp.isAnchor)
					{
						for (int j = 0; j < sps.anchorRefsList1[sp.viewId].size(); ++j)
							if (sps.anchorRefsList1[sp.viewId][j] == sp.list1[i].second)
							{
								vId = j;
								break;
							}
					...
					else
					{
						for (int j = 0; j < sps.nonAnchorRefsList1[sp.viewId].size(); ++j)
							if (sps.nonAnchorRefsList1[sp.viewId][j] == sp.list1[i].second)
							{
								vId = j;
								break;
							}
				...
				{
					if (sp.list1[i].first.second>picNum)
					{
						WriteUvlc(RPLR_POS);
						WriteUvlc(sp.list1[i].first.second - picNum - 1);
					}
					else
					{
						WriteUvlc(RPLR_NEG);
						WriteUvlc(picNum - sp.list1[i].first.second - 1);
					}
					picNum = sp.list1[i].first.second;
				}
			}
			WriteUvlc(RPLR_END);
		}
...
void PadBits(void *buf1, int bl, void *src1, int sl, int size)
{
	...
	if (sl != 0)
	{
		...
		{
			uint32_t _and = (1<<nd)-1;
			old = (old<<nd)|((now>>(hv-nd))&_and);
#ifndef ARCH_IS_BIG_ENDIAN
			BSWAP(old);
#endif
			*buf = old;
...
void H264toMVCPacker::appendNAL(char *buf, int size)
{
	...
	case NAL_UNIT_SPS:
		...
		sps.viewCodingOrder.push_back(0);
		sps.viewCodingOrder.push_back(1);
		sps.anchorRefsList0[1].clear();
		sps.anchorRefsList0[1].push_back(0);
		sps.nonAnchorRefsList0[1].clear();
		sps.nonAnchorRefsList0[1].push_back(0);
		state = 1000;
		break;
	...
	case NAL_UNIT_CODED_SLICE_IDR:
		...
		cout << sp.timeId << ":";
		for (int i = 0; i < sp.list0.size(); ++i)
			cout << "(" << sp.list0[i].first.first << "," << sp.list0[i].second << "),";
		for (int i = 0; i < sp.list1.size(); ++i)
			cout << "(" << sp.list1[i].first.first << "," << sp.list1[i].second << "),";
		cout << endl;
		sp.viewId = sp.timeId / 2 & 1;
		sp.timeId = sp.timeId / 4 * 4;

		for (int i = 0; i < sp.list0.size(); ++i)
		{
			sp.list0[i].second = sp.list0[i].first.first / 2 & 1;
			sp.list0[i].first = makePair(sp.list0[i].first.first/4*4, sp.list0[i].first.second);
		}

		for (int i = 0; i < sp.list1.size(); ++i)
		{
			sp.list1[i].second = sp.list1[i].first.first / 2 & 1;
			sp.list1[i].first = makePair(sp.list1[i].first.first/4*4, sp.list1[i].first.second);
		}

		cout << "(" << sp.timeId << "," << sp.viewId << "):";
		for (int i = 0; i < sp.list0.size(); ++i)
			cout << "(" << sp.list0[i].first.first << "," << sp.list0[i].second << "),";
		for (int i = 0; i < sp.list1.size(); ++i)
			cout << "(" << sp.list1[i].first.first << "," << sp.list1[i].second << "),";
...
int H264toMVCPacker::takeNAL(char *buf, int size)
{
	...
	case 3000:
		...
		++state;
	case 3001:
		if (sp.IDR && !firstSubSps)
		{
			firstSubSps = true;
			BitstreamInit(&bs, packerBuf, sizeof(packerBuf));
			this->writeSubsetSequenceParameterSet();
			uint8_t*dst = Finallize(len);
			if (len <= size)
				memcpy(buf, dst, len);
			break;
	...
	case 3003:
		...
		{
			BitstreamInit(&bs, packerBuf, sizeof(packerBuf));
			this->writePictureParameterSet(1);
			uint8_t*dst = Finallize(len);
			if (len <= size)
				memcpy(buf, dst, len);
			break;
	...
	case 3005:
		{
			...
			{
				int newoff = ((off-1)|7)+1;
				int bits = (1<<(newoff-off))-1;
				off = newoff;
				dst[len-1] |= bits;
				lastCstart = ((lastCstart-1)|7)+1;
			}
			PadBits(dst, off, lastC, lastCstart, lastCsize*8);
			off = off + (lastCsize*8-lastCstart);
			len = (off+7)/8;
			//len += lastCsize;
			if (len <= size)
				memcpy(buf, dst, len);
	...
	++state;
	while (len>0 && len<=size && buf[len-1]==0)
		--len;
	return len;
}
